...
## Design Decisions
- **Photos**: Homepage photo strip currently uses Action1-3, Action6-8, Action10 plus IDE1 + IDE2 (MIT IDE Conference, April 2026). Photos are color as of 2026-04-16 (grayscale filter removed). Subpage headers use backdrop images from `public/images/backdrops/`. Do NOT use Action5.jpg anywhere. Action4 is also omitted (too dark). Source photos in `../Photos/` are full-res (4128×2752, several MB); resize to ~1600px wide with `sips --resampleWidth 1600 -s formatOptions 80` before placing in `public/images/`.
...
| Date | Changes |
|------|---------|
| 2026-08-31 | Contact modal: replaced scheduling and administrative support contact Sumaiya Rahman Haddad with Cindy Pham (`phamcl@mit.edu`). Removed the former contact's phone number because no replacement number was provided. |
...
| 2026-05-01 | Added new featured paper "Breaking Bad Financial Habits: How LLM Conversations Correct Financial Misconceptions" (Ross, So, Lo) to `featured-research.ts` at the top — flows to both home page marquee and research page sticky-intro reveal. Mirrored entry in `search-index.ts` (working-papers section). RolesSection: added Fuller & Thaler Asset Management ("Advisor") row at the bottom of Leadership & Affiliations, linked to https://fullerthaler.com/. |
| 2026-05-01 | Removed "Losing is Optional" (de Silva, Smith, So) from `featured-research.ts` per Eric's direction to drift the featured rotation toward AI papers over time. Paper still listed on `research.astro` and in `search-index.ts` as a published entry. |
| 2026-05-01 | Reordered featured: Breaking Bad Financial Habits slotted into position 2 (Losing is Optional's old spot) instead of #1. Then dropped Box Jumping; featured rotation now AI-only (AI Advisors → Breaking Bad Financial Habits → Fiscal Frontier). Added new "Artificial Intelligence" category at the top of `research.astro`'s `categories` array, listing the same three papers. Moved Fiscal Frontier out of the Behavioral and Financial Economics array to avoid a duplicate entry. |
| 2026-05-06 | TeachingSection: course cards on the homepage are now buttons that open a centered modal (overlay + dialog inline in the same component) with course details. Two panels — AI Applications in Business (15.236 GBAIR; spring H3, co-taught with Simon Johnson and Jonathan Ruane; three-part arc covering The Worker, The Firm, Robots/Policy/Practice) and Investment Decisions (Alphanomics; spring full term; topics list covering active investing, alpha, value investing, investor psychology, sentiment, fraud detection, AI). Modal has Escape-to-close, overlay click-to-close, focus return, body scroll lock, and rebinds on `astro:page-load`. Source content drawn from `Teaching/AI/Delivered Classes/GBAIR2026/15.236 GBAIR Syllabus 2026 V3.docx` and `Teaching/AlphaMIT/Syllabus_20260127.docx`. Per Eric: dates kept to semester only — no specific years or class times. |
| 2026-05-06 | Refocused AI Applications modal: leads with three sections (How the technology works / The frontier of AI / How to work with AI) ahead of the original three-part course arc. Then added a third course "AI Foundations" — summer orientation course; sections cover how modern AI works, working with the tools (vibe coding, hands-on labs), and the science of cognitive outsourcing. Source: `Teaching/AI/Delivered Classes/AI Foundations/AI Foundations 202604/AI Foundations - Course Proposal Draft.docx`. RolesSection: Fuller & Thaler title changed Advisor → Researcher. |
| 2026-05-06 | Featured research now has a click-to-open detail modal mirroring the course pattern. New shared component `site/src/components/FeaturedResearchModal.astro` renders one panel per paper with eyebrow/title/authors/TLDR, optional Abstract section, and a "Read the paper" button at the bottom (or "Full paper coming soon" when no link is provided). Triggered by any element with `data-paper-trigger="<slug>"`. Wired into the homepage marquee (`FeaturedResearch.astro` — anchors converted to buttons) and the Research page sticky-intro reveal cards (`research.astro` — anchors converted to buttons, .reveal-card CSS updated for button defaults). Data shape extended: `featured-research.ts` now has `slug` (required, stable id), `tldr` (required), and optional `abstract`; `link` made optional. Abstracts populated for Breaking Bad Financial Habits (canonical, from `Projects/AIF/Drafts/Draft_20260422.pdf`) and Fiscal Frontier (paraphrased from the public Brookings page); AI Advisors abstract left as TLDR-only pending Eric supplying the canonical text. README updated with featured-paper workflow; CLAUDE.md updated with the same. |
| 2026-05-07 | CV reformulated to be more AI-forward without overclaiming. Master renamed `main_20260417.tex` → `main_20260507.tex`. Header date refreshed to May 2026. Research Interests reordered: "Artificial intelligence, asset pricing and trading, behavioral economics, regulatory policy" — AI moved to lead, "market frictions" cut. MIT branch of Academics now lists three AI leadership roles (Faculty Co-Director, AI Executive Academy 2024-Present; Lead Faculty, MIT Sloan Generative AI Hub for Teaching and Learning 2023-Present; Research Group Lead, AI in Financial Markets and Decision-Making at MIT IDE) plus a Teaching line (AI Foundations; AI Applications in Business / 15.236 GBAIR; Investment Decisions / Alphanomics). The two AI roles already in Professional Service → MIT Sloan were removed to avoid duplication. Section header renamed `PUBLISHED, FORTHCOMING, & ACCEPTED RESEARCH` → `RESEARCH`, with new "Topic: Artificial Intelligence" added at the top containing Lost in Context (forthcoming ICML 2026) and The Fiscal Frontier (Brookings, Feb 2025). Recompiled and synced PDF to `site/public/files/CV_EricSo.pdf`. |
| 2026-06-15 | Photo strip: added Action8 and Action10 to the homepage scroll (resized 4128×2752 → 1600px wide via `sips`, 5.5MB→429KB and 3.9MB→245KB). The Collision page: added a featured promo video section between the book hero (overview) and the chapter breakdown. Renders via a YouTube iframe embed gated on a `promoVideoId` const in the frontmatter (set to `yJsR-XirDqE`) — raw `Photos/PromoVideov1.mp4` is 321MB and exceeds GitHub's 100MB file limit, so the video lives on YouTube (`Photos/*.mp4` gitignored). Section is dark-themed, 16:9 responsive, lazy-loaded. To swap the video later, change `promoVideoId`. |
| 2026-05-07 | Second CV pass. IDE role shortened to one line: "Research Group Lead, AI, Finance & Decisions Group, MIT IDE" (matches website RolesSection phrasing). Teaching line collapsed to actual class names: "AI Foundations, Alphanomics, Global Business of AI & Robotics" — drop the descriptive subtitles per Eric. AI topic in RESEARCH expanded to mirror the website's full featured-research set: now lists all four featured papers in website order (Lost in Context → AI Advisors and the Competence-Judgment Tradeoff → Breaking Bad Financial Habits → The Fiscal Frontier). The two AI working papers (AI Advisors, Breaking Bad Financial Habits) moved from `research_in_progress.tex` into the AI topic; that file now contains only "Fee the People". New standalone "BOOK" section added between Academics and Research, listing "The Collision: What AI Does to Us" with a link to the website book page. PDF recompiled and synced. |
| 2026-07-15 | Added two AI papers. **Beyond Accuracy: Can LLM Forecasters Profit on Prediction Markets?** (Henry, Ross, Marzoev, So, Lo — accepted Spotlight at the ICML Workshop on Forecasting as a New Frontier of Intelligence, 2026; no link yet) added to `research.astro` AI category (after Emergent Negligence) + `search-index.ts` — list-only, not featured. **Emergent Negligence** (So, solo) status upgraded working paper → **Accepted at COLM 2026** across `research.astro`, `search-index.ts`, and CV; promoted into the featured rotation in `featured-research.ts`, replacing The Fiscal Frontier at its array index (Fiscal Frontier remains on the research page, just no longer featured). CV: master renamed `main_20260617` → `main_20260715`, header date June → July 2026, both papers added/updated under Topic: Artificial Intelligence. Compiled with **tectonic** (no pdflatex/latexmk on this machine); fixed a latent malformed `\vskip %-0.25cm` (dimension commented out) on the RESEARCH rule line that caused tectonic's error-recovery to swallow the "Topic: Artificial Intelligence" header — commented the whole command. Recompiled, synced PDF to `site/public/files/CV_EricSo.pdf`, built, committed, pushed. |
| 2026-07-28 | Removed two AI papers per Eric. **The Compression Hypothesis: Uncovering Predictive Signal in LLM Forecasts** (Renda, Ross, So) removed from `working-papers.astro`, `research.astro` (AI category), `search-index.ts`, and CV. **Beyond Accuracy: Can LLM Forecasters Profit on Prediction Markets?** (Henry, Ross, Marzoev, So, Lo) removed from `research.astro` (AI category), `search-index.ts`, and CV. CV master renamed `main_20260715.tex` → `main_20260728.tex` (header date stays July 2026), recompiled with tectonic, verified both titles absent via pypdf, synced PDF to `site/public/files/CV_EricSo.pdf`. Built, committed, pushed. |
