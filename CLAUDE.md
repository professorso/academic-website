...
| 2026-06-15 | Photo strip: added Action8 and Action10 to the homepage scroll (resized 4128×2752 → 1600px wide via `sips`, 5.5MB→429KB and 3.9MB→245KB). The Collision page: added a featured promo video section between the book hero (overview) and the chapter breakdown. Renders via a YouTube iframe embed gated on a `promoVideoId` const in the frontmatter (empty for now → section hidden) — raw `Photos/PromoVideov1.mp4` is 321MB and exceeds GitHub's 100MB file limit, so per Eric the video goes to YouTube and the id gets pasted into `promoVideoId`. Section is dark-themed, 16:9 responsive, lazy-loaded. |
